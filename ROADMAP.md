...
- [ ] Mission completion fanfare — when an Airport Hop / Touch & Go / Ring Chase mission completes, play a quick three-note ascending chime (sine triad) and flash a centered "MISSION COMPLETE" banner with the final time/score for ~3s. Right now scored runs just end with a status banner that auto-fades; a real audio sting + ceremonial banner gives the player a payoff moment that matches the effort. Reuses existing WebAudio nodes (~30 LOC, no assets).
- [ ] Fuel gauge warning at 20% — when fuel drops below 20%, the HUD fuel readout flashes amber and pushes a one-shot ATC line ("Fuel state low, recommend divert"). Below 10% it flashes red. Currently the only fuel feedback is the bare percentage in the HUD and the eventual ENGINE OUT slam; a graded warning gives the player time to actually divert instead of pancaking deadstick onto a meadow.
- [ ] Cockpit head-bob — in cockpit cam only, add a subtle low-frequency bob to the camera position tied to airspeed + ground roughness (more bob on taxi, less in smooth cruise, a sharp jolt on touchdown). Tiny perlin-ish offset on the camera target so saved photos stay clean. Sells the "I'm sitting in the plane" feeling without touching physics.
